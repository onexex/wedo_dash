<?php
    ...
    /* Map a status description to a themed pill colour. Guarded so it coexists
       ...
       NOTE: the danger tests run BEFORE the approve test on purpose — the string
       "Disapproved" contains "approve", so checking approve first would colour a
       disapproval green. */
...
<head>
    ...
    <!-- Print the report area in landscape. Hooks preserved:
         #btnprint #tblprint #tab #tabth #darviewer .captionText #captionText #dateRange #empcompid #dtp1 #dtp2 -->
...
<body>
    <?php $wd_active = 'obviewer'; include 'includes/wd-header.php'; ?>
    ...
    <div class="wd-pagehead">
        <div>
            <h1>Official Business Trip Viewer</h1>
            <p>Review filed Official Business trips across employees and date ranges.</p>
        </div>
    </div>

    <section class="wd-card">
        <div class="wd-card__head">
            <h3>Official Business reports</h3>
            <form id="vlilodata" style="display:flex;align-items:flex-end;gap:10px;flex-wrap:wrap;margin:0">
                ...
                <button class="wd-btn wd-btn--ghost" id="obvi" type="button" title="Refresh"><i class="fa-solid fa-rotate"></i></button>
            </form>
        </div>

        <div id="tblprint">
            <!-- print-only captions (shown by #btnprint handler) -->
            <label class="captionText d-none" id="captionTextMain">Official Business Report</label>
                ...
                <table class="wd-table" id="tab">
                    ...
                    <tbody id="darviewer">
                        <?php
                            ...
                            $ids = 0;
                            if (isset($statement) && !empty($statement->rowCount())) {
                        ...
                        <tr>
                            ...
                            <td><?php echo date("F j, Y", strtotime($row["Filing_Date"])); ?></td>
                            <td><?php echo date("F j, Y", strtotime($row["OBDateFrom"])); ?></td>
                            <td><?php echo date("F j, Y", strtotime($row["OBDateTo"])); ?></td>
                            <td><?php echo htmlspecialchars($row["Itinerary_From"]); ?></td>
                            <td><?php echo htmlspecialchars($row["Itinerary_To"]); ?></td>
